...
void simplePlot (const std::vector<double> &x, IplImage **dst) {
    ...
    for (int j = 0; j < (x.size() - 1); ++j) {
        cvLine(*dst, cvPoint(xmult*j, 480 - ymult * (x[j] - min)), cvPoint(xmult*(j+1), 480 - ymult * (x[j+1] - min)), cvScalarAll(255));
    }
    cvLine(*dst, cvPoint(0, 480 + ymult*min), cvPoint(640,480+ymult*min),cvScalarAll(255));
