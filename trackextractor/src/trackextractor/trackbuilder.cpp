...
void TrackBuilder::extractionImage(IplImage** dst) {
    _TICTOC_TIC_FUNC;
...
int TrackBuilder::toDisk(FILE* f) {
    ...
    if (fwrite(&code, sizeof(int), 1, f) != 1) { return -1; }
    int ntracks = activetracks.size() + finishedtracks.size();

    if (fwrite(&ntracks, sizeof(int), 1, f) != 1) { return -1; }
...
int TrackBuilder::loadTracksFromDisk(FILE* f, TrackPoint *(*ptLoader) (FILE *f),int minLenToLoad) {
    ...
    int code =getTypeCode(); // THIS IS A FUCKIN GUESS. I HAVE NO IDEA WHAT MARC INTENDED HERE
    cout << "code = " << code;
